...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    return await ctx.db.query('mindsetMoments').collect();
  },
});

// Seed mindset moments (admin/dev only)
export const seed = mutation({
  args: {},
  handler: async (ctx) => {
    // Check if already seeded
    const existing = await ctx.db.query('mindsetMoments').first();
    if (existing) {
      return { seeded: false, message: 'Mindset moments already exist' };
    }
    ...
    return { seeded: true, count: quotes.length };
  },
});

// Seed daily challenges (admin/dev only)
export const seedChallenges = mutation({
  args: {},
  handler: async (ctx) => {
    // Check if already seeded
    const existing = await ctx.db.query('dailyChallenges').first();
    if (existing) {
      return { seeded: false, message: 'Daily challenges already exist' };
    }
